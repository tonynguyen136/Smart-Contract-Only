...
contract Auction {
    ...
    // Register new Bidder
    function register(address _account, uint8 _token) public onlyAuctioneer stateCheck(State.CREATED){ 
    ...
    // Start the session.
    function startSession() public onlyAuctioneer stateCheck(State.CREATED){
        state = State.STARTED;
    }
    

    
    function bid(uint8 _price) public stateCheck(State.STARTED){
        ...
        if(currentBidder.token + currentBidder.deposit < _price) revert();
    ...
    function announce() public onlyAuctioneer stateCheck(State.STARTED){
    ...
    function getDeposit() public stateCheck(State.CLOSING){
